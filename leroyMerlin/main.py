...
def get_window():
    column_to_be_centered = [
        [sg.T("", key='-TST-')],
        [sg.Text("Файл для парсинга: "), sg.Input(size=(60, 1), enable_events=True, key='-FILENAME-'), sg.FileBrowse(button_text='Выбрать', key="-IN-", file_types=(("Excel", "*.xlsx"), ))],
        [sg.T("Оценка объема:")],
        ...
        [sg.Multiline(key='-ML-'+sg.WRITE_ONLY_KEY, size=(145, 40), auto_refresh=True)]
    ]
    ...
    window = sg.Window(title='Поиск цен на Leroymerlin', layout=layout, size=(600, 450), icon=icon)
